...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < nthreads; i++) {  // line:conc:psum:createloop1
        ...
        Pthread_create(&tid[i], NULL, sum,
                       &myid[i]);    // line:conc:psum:createloop3
...
{
    int myid = *((int *) vargp);
        /* Extract the thread ID */  // line:conc:psum:extractid
    long start = myid * nelems_per_thread;
        /* Start element index */  // line:conc:psum:getstart
    long end = start + nelems_per_thread;
        /* End element index */  // line:conc:psum:getend
    long i, sum = 0;
